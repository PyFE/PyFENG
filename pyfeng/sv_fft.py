...
class RoughHestonFft(rheston.RoughHestonABC, FftABC):
    ...
    def mgf_logprice(self, uu, texp):
        ...
        def F(a,x):
            """
           Left hand side of fractional Riccati equation
        ...
        def L(aa,t):
            ...
            for i in range(0,len(aa)):
                ...
                for j in range(1,k + 1):
                    ...
                    h_hat[j] = np.dot(self.a_kp1(j-1,delta),F_a_h_hat[0:j]) + self.a_j_kp1(j,delta,j + 1) * F(aa[i],h_hat_p[j])
...
class RoughHestonFastHybridFft(rheston.RoughHestonABC, FftABC):
    """
    Rough Heston model option pricing with FFT using Fast Hybrid Approximation
