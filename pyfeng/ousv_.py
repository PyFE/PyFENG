...
class OusvCondMC(sv.SvABC, sv.CondMcBsmABC):
    ...
    def _bm_incr(self, tobs, mr, cum=False, n_path=None):
        """
        ...
        bm_var = np.exp(2 * mr * tobs) - np.exp(2 * mr * tobs_lag)
    ...
    def vol_paths(self, tobs, mr, theta=0):
        """
        ...
        """
        bm_path = self._bm_incr(tobs, mr, cum=True)
        sigma_t = theta + (self.sigma - theta + self.vov / np.sqrt(2 * mr) * bm_path) * np.exp(-mr * tobs[:, None])
        sigma_t = np.insert(sigma_t, 0, np.array([self.sigma] * sigma_t.shape[1]), axis=0)
        return sigma_t

    def cond_fwd_vol(self, texp, mr, theta=0):
        """
        ...
        tobs = self.tobs(texp)
        n_dt = len(tobs)
        sigma_paths = self.vol_paths(tobs, mr, theta)
        ...
        fwd_cond = np.exp(self.rho * ((sigma_final**2 - self.sigma**2) / (2 * self.vov) - self.vov * 0.5 * texp -
                                      mr * theta / self.vov * int_sigma +
                                      (mr/self.vov - self.rho * 0.5) * int_var))   # scaled by initial value

        vol_cond = rhoc * np.sqrt(int_var / texp)

        return fwd_cond, vol_cond

    def price(self, strike, spot, texp, mr, theta=0, cp=1):
        """
        ...
        for t in texp:
            ...
            fwd_cond, vol_cond = self.cond_fwd_vol(t, mr, theta)
